...
int main()
{
   struct node *head_start, *node1, *new_node_at_end, *temporary1, *temporary2, *temporary3;
   ...
   printf("\nInserting a new node at the end of the circular singly linked list");
   printf("\nEnter the data for new node:");
   scanf("%d",&new_data_for_new_node);
   new_node_at_end = malloc(sizeof(struct node));
   ...
   temporary3->next=new_node_at_end;
   new_node_at_end->next=head_start; 
   printf("\n After inserting a new node at end, the circular list : \n");
   n = 1;
   if(head_start == NULL)
   {
      printf("List is Empty\n");
